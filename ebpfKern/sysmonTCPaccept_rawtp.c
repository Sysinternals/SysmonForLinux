...
#include "sysmonTCPaccept.c"

SEC("sysmon/TCPaccept/rawExit")
__attribute__((flatten))
int TCPacceptRawExit(struct bpf_our_raw_tracepoint_args *ctx)
{
    ...
    // only handle X events
    if (eventArgs->syscallId != __NR_accept && eventArgs->syscallId != __NR_accept4) {
        return 0;
    }
    ...
    ptr = set_TCPaccept_info(eventHdr, config, pidTid, cpuId, eventArgs);
