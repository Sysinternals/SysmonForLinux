...
SEC("sysmon/FileDeleteAtCwd/rawExit")
__attribute__((flatten))
int FileDeleteAtCwdRawExit(struct bpf_our_raw_tracepoint_args *ctx)
{
    ...
    // only handle unlinkat events where DFD is AT_FDCWD
    if (eventArgs->syscallId != __NR_unlinkat || eventArgs->a[0] != (uint32_t)AT_FDCWD) {
        return 0;
    }
